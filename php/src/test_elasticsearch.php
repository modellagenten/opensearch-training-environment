 ...
 * 
 * Dieses Skript testet die Verbindung zu Elasticsearch und zeigt
...
function testElasticsearchConnection(): bool
{
    try {
        // Erstelle Elasticsearch Client mit Basic Authentication
        $client = ClientBuilder::create()
            ->setHosts([$_ENV['ELASTIC_HOST']])
            ->setBasicAuthentication($_ENV['ELASTIC_USER'], $_ENV['ELASTIC_PASSWORD'])
            ->build();
        
        echo "🔍 Teste Elasticsearch Verbindung...\n";
        echo "Host: " . $_ENV['ELASTIC_HOST'] . "\n";
        echo "User: " . $_ENV['ELASTIC_USER'] . "\n";
        echo str_repeat("-", 50) . "\n";
        ...
        echo "✅ Verbunden mit Elasticsearch\n";
        ...
        echo "❌ Fehler bei Elasticsearch Verbindung:\n";
        echo "   " . $e->getMessage() . "\n";
        echo "\n💡 Mögliche Lösungen:\n";
        echo "   1. Stellen Sie sicher, dass Elasticsearch läuft (docker-compose up)\n";
...
{
    echo str_repeat("=", 60) . "\n";
    echo "ELASTICSEARCH VERBINDUNGSTEST\n";
    echo "Schulungsumgebung - PHP (" . PHP_OS . ")\n";
    echo str_repeat("=", 60) . "\n";
    
    $success = testElasticsearchConnection();
